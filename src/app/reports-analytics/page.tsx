...
export default function AnalyticsPage() {
  ...
  const safetyTypeData = useMemo(() => {
    if (!safetyEvents) return [];
    const counts = safetyEvents.reduce((acc: any, e) => {
  ...
  const laborTrendData = useMemo(() => {
    ...
    return [...dailyReports]
      ...
      .slice(-10)
      .map(r => {
        const totalHrs = (r.manHours || []).reduce((sum: number, mh: any) => sum + (mh.headcount * mh.hours), 0);
        return {
      ...
      return (
        <ResponsiveContainer width="100%" height={350}>
          <AreaChart data={laborTrendData}>
            ...
            <YAxis axisLine={false} tickLine={false} className="text-[10px]" />
            <Tooltip content={<ChartTooltipContent />} />
            <Area type="monotone" dataKey="incidents" stroke="#ef4444" fillOpacity={1} fill="url(#colorInc)" strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      );
    }
      ...
      return (
        <ResponsiveContainer width="100%" height={350}>
          <LineChart data={laborTrendData}>
            ...
            <YAxis axisLine={false} tickLine={false} className="text-[10px]" />
            <Tooltip content={<ChartTooltipContent />} />
            <Line type="stepAfter" dataKey="hours" stroke="#FF9800" strokeWidth={3} dot={{ r: 4, fill: '#FF9800' }} />
          </LineChart>
        </ResponsiveContainer>
      );
    }
    ...
    <div className="flex flex-col h-[calc(100vh-100px)] gap-4 animate-in fade-in duration-700">
            ...
            <div className="pt-4 border-t space-y-4">
              <div className="p-3 bg-white rounded-sm border border-slate-100 shadow-sm">
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed italic">
                  {activeModel === 'project-pulse' && "Analyzing global distribution of work orders across the portfolio states."}
                  {activeModel === 'safety-matrix' && "Monitoring recordable incident frequency vs temporal site progression."}
                  {activeModel === 'labor-dynamics' && "Step-based tracking of cumulative man-hours reported in daily logs."}
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-auto">
        <Card className="rounded-sm border-slate-200 shadow-sm p-4 bg-[#46a395] text-white overflow-hidden relative group cursor-default">
          ...
          <p className="text-[9px] mt-2 font-bold opacity-70">Aumento del 12% vs mes anterior</p>
        </Card>
        ...
        <Card className="rounded-sm border-slate-200 shadow-sm p-4 bg-white overflow-hidden relative group cursor-default border-l-4 border-l-[#FF9800]">
          ...
          <p className="text-[9px] mt-2 font-bold text-orange-500 flex items-center gap-1">
            <TrendingUp className="h-3 w-3" /> 85% de cierre de hallazgos
          </p>
        </Card>
        ...
        <Card className="rounded-sm border-slate-200 shadow-sm p-4 bg-white overflow-hidden relative group cursor-default border-l-4 border-l-slate-800">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Man-Hours</p>
          <h3 className="text-3xl font-black mt-1 text-slate-800">
            {laborTrendData.reduce((acc, curr) => acc + curr.hours, 0).toLocaleString()}h
          </h3>
          <p className="text-[9px] mt-2 font-bold text-slate-500">Últimos 10 periodos reportados</p>
        </Card>
      </div>
    </div>
  );
